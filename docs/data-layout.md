...
- Spark local metadata under `data/_spark/warehouse/`
...
- Store vector metadata with model name, version, generation timestamp, dataset snapshot
  references, hardware or device metadata, and embedding-run lineage.
- Treat embeddings as reproducible derivatives; they can be rebuilt from normalized products and images.
- Persist run-level manifests under `data/embeddings/manifests/` or the artifact-specific manifest
  path referenced by the embedding outputs.
...
- Index manifests should reference the exact embedding artifact and embedding run that produced the
...
- Recommendation exposure, click, and feedback logs written through the public API belong under
  `data/reports/experiments/`.

### `data/_spark/`
Purpose:
- Local Spark metadata and warehouse paths used by offline pipeline jobs.
