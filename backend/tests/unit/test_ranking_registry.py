...
class RankingRegistryTests(unittest.TestCase):
    ...
    def test_registers_latest_candidate_model_with_lineage_metadata(self) -> None:
        ...
        latest_candidate = json.loads(outputs["latest_candidate"].read_text(encoding="utf-8"))
        registry_log = read_jsonl(outputs["registry_log"])
        training_manifest = json.loads(
            self.training_outputs["manifest"].read_text(encoding="utf-8")
        )

        self.assertEqual(registration["stage"], "candidate")
        self.assertEqual(registration["source_run_id"], training_manifest["run_id"])
        self.assertEqual(
            registration["lineage"]["model_artifact_path"],
            training_manifest["artifacts"]["model_path"],
        )
        self.assertEqual(
