...
class OnlineFeatureServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.store_root = Path(self.tmp_dir.name) / "features" / "online_bootstrap"
        ...
        self.service = OnlineFeatureService(self.store_root)

    def tearDown(self) -> None:
        shutil.rmtree(Path(self.tmp_dir.name), ignore_errors=True)
        self.tmp_dir.cleanup()

    def test_fetches_session_customer_and_article_payloads(self) -> None:
        session = self.service.get_session_intent_features(
            customer_id="0001",
            session_id="0001-2020-09-20",
        )
        customer = self.service.get_customer_realtime_features(customer_id="0001")
        article = self.service.get_article_realtime_features(article_id="108775015")

        self.assertEqual(session["cart_add_count_30m"], 1)
        self.assertEqual(customer["purchase_count_30d_rt"], 1)
        self.assertTrue(article["is_in_stock_rt"])

    def test_returns_empty_payload_for_missing_keys(self) -> None:
        self.assertEqual(
            self.service.get_customer_realtime_features(customer_id="missing"),
            {},
        )
