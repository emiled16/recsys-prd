...
class RankingEvaluationTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        training_outputs = train_local_ranking_model(
            normalized_root=self.normalized_root,
            indexes_root=self.indexes_root,
    ...
    def test_evaluates_latest_registered_model(self) -> None:
        outputs = evaluate_registered_ranking_model(models_root=self.models_root)

        evaluation = json.loads(Path(outputs["evaluation"]).read_text(encoding="utf-8"))

        self.assertEqual(evaluation["mlflow_model"]["version"], "7")
        self.assertEqual(evaluation["registration_stage"], "candidate")
        self.assertGreaterEqual(evaluation["metrics"]["pairwise_accuracy"], 0.66)
        self.assertEqual(evaluation["dataset"]["row_count"], 9)
