...
def load_customer_profiles(normalized_root: Path) -> dict[str, dict[str, str]]:
    ...
    rows = read_tabular_rows(normalized_root / "customers" / "customers_normalized.parquet")
...
def load_product_catalog(normalized_root: Path) -> dict[str, dict[str, str]]:
    ...
    rows = read_tabular_rows(normalized_root / "products" / "products_normalized.parquet")
...
def load_image_presence(normalized_root: Path) -> dict[str, bool]:
    ...
    rows = read_tabular_rows(normalized_root / "images" / "product_images_manifest.parquet")
